...
function getStatusBadge(shipment) {
    ...
    return `<span class="${badgeClass}">${badgeContent}</span> <button type="button" class="btn btn-info infoBtn" data-bs-toggle="modal" data-bs-target="#infoModal" data-shipment='${JSON.stringify(shipment)}'>i</button>`;
}
...
function displayInfoModal(shipmentData) {
  var modalContent = `
      <p><strong>Nomor Resi:</strong> ${shipmentData.tracking_number}</p>
      <p><strong>Nama Pengirim:</strong> ${shipmentData.sender_name}</p>
      <p><strong>Nama Penerima:</strong> ${shipmentData.intended_receiver_name}</p>
      <p><strong>Alamat Penerima:</strong> ${shipmentData.receiver_address}</p>
      <p><strong>Status:</strong> ${getStatusBadge(shipmentData)}</p>
      <!-- Tambahkan informasi lain sesuai kebutuhan -->`;

  var infoContent = document.querySelector('#infoModal .modal-body');
  infoContent.innerHTML = modalContent;
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  infoButtons.forEach(function (btn) {
      btn.addEventListener('click', function () {
          var shipmentData = JSON.parse(btn.getAttribute('data-shipment'));
          displayInfoModal(shipmentData);
      });
  });
});
